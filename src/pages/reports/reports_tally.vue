...
const querySales = async () => {
    ...
    try {
        ...
        tally.value = response.data;

        try {
            const response = await axios.get(`http://127.0.0.1:8000/api/reports/totaltally/${date1.value}/${date2.value}`);
            const data = response.data;
            totaltallies.value = data[0].TotalSales;

            try {
                const response = await axios.get(`http://127.0.0.1:8000/api/total/tallytransactions/${date1.value}/${date2.value}`);
                const data = response.data;
...
const filteredTally = computed(() => {
    if (Array.isArray(tally.value)) { 
        return tally.value.filter(row => row.Alias.toLowerCase().includes((filters.value.global.value || '').toLowerCase()));
    }
    return [];
});

const totalSum = computed(() => {
    if (Array.isArray(filteredTally.value)) { 
        return filteredTally.value.reduce((sum, row) => sum + (row.Total || 0), 0);
    }
    return 0;
});
...
<template>
    <div class="px-5 relative md:px-[43px] font-sora select-none top-[80px] h-[calc(100vh-80px)] overflow">
        <div class="flex flex-col gap-2 h-full">
            <p class="text-[40px] font-bold mb-5">Tally</p>
            <div class="p-5 border-2 rounded-xl ">
                <p class="text-[24px] font-bold">Filter By:</p>
                <div class="flex flex-col  sm:flex-row sm:items-center gap-2">
                    <p>From</p>
                    <Calendar v-model="Fromdate" :manualInput="false" showButtonBar
                        :invalid="err" />
                ...
                <div class="overflow-scroll h-[calc(100vh-500px)] mt-5">
                    <DataTable v-model:filters="filters" :value="filteredTally" removableSort
                        tableStyle="min-width: 50rem" paginator :rows="10" :rowsPerPageOptions="[5, 10, 15]">
                        <template #header>
                        ...
                        <Column field="Alias" header="Customer Name" sortable class="w-[30%] md:w-auto">
                            <template #body={data}>
                                <p class="font-bold"> {{ data.Alias }}</p>
                            </template>
                        </Column>
                        <Column v-if="isDesktop" field="AdditionalFee" header="Additional Fees" sortable>
                            <template #body={data}>
                                <p class="font-bold">₱ {{ data.AdditionalFee }}</p>
                            </template>
                        </Column>
                        <Column v-if="isMobile" field="Notes" header="Notes" sortable></Column>
                        <Column field="Total" header="Total" sortable>
                            <template #body="{data }">
                                <p class="font-bold text-primary">₱ {{ data.Total }}</p>
                            </template>
                            <template #footer>
                                <div class="font-bold text-primary">Total: ₱ {{ totalSum }}</div>
                            </template>
                        </Column>
                        <template #empty>
            ...
            <div class="flex flex-col sm:flex-row gap-5 items-center justify-center  w-full">
                <div class="flex flex-col justify-center h-[150px] w-[400px] bg-primary rounded-lg p-5">
                    ...
                    <span class="flex justify-center items-center gap-3">
                        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="white" class="w-clamp2">
                            <path
                ...
                <div class="flex flex-col justify-center h-[150px] w-[400px] bg-secondary rounded-lg p-5">
                    <p class="text-clamp2 font-semibold text-center">Total Debt</p>
                    <p class="text-clamp5 font-bold text-accent text-center">₱ {{ totaltallies }}.00</p>
                </div>
            </div>
        </div>
